...
public class DNAAnalysisController {
    ...
    @ApiOperation("Evaluate DNA sequence")
    @ApiResponses(
            value = {
                    @ApiResponse(code = 200, message = "The DNA sequence belongs to a mutant"),
                    @ApiResponse(code = 403, message = "The DNA sequence belongs to a human")
            }
    )
    @PostMapping("/")
    public ResponseEntity<?> isMutant(@RequestBody @Validated DNASequenceDTO sequenceDTO)
